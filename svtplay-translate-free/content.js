 console.log("SVTPlay-Translate content script started");
						...
						if (content === lastElementChange) { 
							...
							if (content.includes("width:100%;height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;")){
								...
								(async () => {
									...
									if (response) {
										if (entireElement){
											addedNode.innerHTML=response;
											lastElementChange=response;
										} else {
											//addedNode.innerHTML=foundvod[1]+"<span>"+response.translatedText+"</span>"+foundvod[3];
											addedNode.innerHTML=addedNode.innerHTML.replace(">"+subtitle+"<",">"+response+"<");;
								...
								if (content.includes("rxp-texttrack-region")){
									...
									if (founddouble){
										...
										(async () => {
												...
												addedNode.innerHTML=addedNode.innerHTML.replace(">"+subtitle1+"<",">"+response1+"<");
												if (response1) {
													...
													if (response2) {														
														addedNode.innerHTML=addedNode.innerHTML.replace(">"+subtitle2+"<",">"+response2+"<");
														lastElementChange=addedNode.innerHTML;
													};// else do nothing - no translation returned
												};	// else do nothing - no translation returned					
											})();
